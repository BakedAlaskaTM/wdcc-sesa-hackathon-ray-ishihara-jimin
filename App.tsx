...
type Screen = 'welcome' | 'profile' | 'lobby' | 'history' | 'map' | 'scores' | 'bill' | 'summary' | 'demo' | 'demoLobby';
...
export default function App() {
  ...
  if (screen === 'welcome') return <WelcomeScreen onStart={() => navigate('profile')} />;
  if (screen === 'profile') return withBackButton(<ProfilePickerScreen onSelect={(selectedProfile) => { setProfile(selectedProfile); setSessionHistory(historyForProfile(selectedProfile)); navigate('lobby'); }} />, goBack);
  if (screen === 'bill') return withBackButton(<PhoneUsageBillScreen displayName={gamePlayerName} lobbyName={gameLobbyName} userId={playerId} roomCode={gameRoomCode} onSessionEnded={(players, timeline, timelineRange) => { setFinalPlayers(players); setActivityTimeline(timeline); setTimelineLabels(timelineRange ? { start: formatTimelineTime(timelineRange.startedAt), end: formatTimelineTime(timelineRange.endedAt) } : null); navigate('summary'); }} />, goBack);
  if (screen === 'summary') return withBackButton(<SessionSummaryScreen players={finalPlayers} activityTimeline={activityTimeline} lobbyName={gameLobbyName} paymentUserId={playerId} roomCode={gameRoomCode} timelineLabels={timelineLabels ?? undefined} onHome={returnHome} />, goBack);
  if (screen === 'demoLobby') return <DemoLobbyScreen onBack={goBack} onStart={() => navigate('demo')} />;
  if (screen === 'demo') return withBackButton(<DemoBillScreen onHome={returnHome} onEnd={(players, timeline) => { setFinalPlayers(players); setActivityTimeline(timeline); setTimelineLabels({ start: '6:14 PM', end: '8:04 PM' }); navigate('summary'); }} />, goBack);
  if (screen === 'history') return <HistoryScreen groups={sessionHistory} profileName={profile ?? 'Your'} onBack={goBack} onOpenGroup={(group) => { setSelectedGroup(group); navigate('scores'); }} onOpenMap={() => navigate('map')} />;
  if (screen === 'map') return <MapScreen history={sessionHistory} profileName={profile ?? ''} onBack={goBack} />;
  if (screen === 'scores' && selectedGroup) return <GroupScoreScreen groupName={selectedGroup.name} members={selectedGroup.members} onBack={goBack} />;
  return <StackDetectorScreen userId={playerId} onGameStarted={(roomCode, displayName, lobbyName) => { setGameRoomCode(roomCode); setGamePlayerName(displayName); setGameLobbyName(lobbyName); navigate('bill'); }} onOpenBill={() => navigate('bill')} onOpenDemo={() => navigate('demoLobby')} onOpenHistory={() => navigate('history')} />;
}
