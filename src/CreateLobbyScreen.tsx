...
export function CreateLobbyScreen({ code, error, isLoading, onChangeCode, onCreate, onDemo, onJoin, onOpenHistory, onOpenMap, onOpenStats }: Props) {
    ...
    <View style={[styles.content, isWeb && styles.phoneFrame]}>
      <View>
        ...
        <Pressable disabled={isLoading} onPress={onCreate} style={({ pressed }) => [styles.createButton, pressed && styles.pressed]}>
          ...
          <View>{isLoading ? <ActivityIndicator color="#2E2A3A" /> : <><Text style={styles.createTitle}>create a stack</Text><Text style={styles.createCaption}>start a new session</Text></>}</View>
        </Pressable>
        ...
        <Pressable disabled={!onOpenMap} onPress={onOpenMap} style={styles.mapCard}><Text style={styles.mapTitle}>map</Text><Text style={styles.mapHint}>see every saved session</Text></Pressable>
