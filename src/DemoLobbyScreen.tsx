...
export function DemoLobbyScreen({ onBack, onStart }: { onBack: () => void; onStart: () => void }) {
  ...
  const liftedPhoneCount = Math.max(0, players.length - stackedCount);
      ...
      <ScrollView contentContainerStyle={[styles.content, isWeb && styles.phoneFrame]}>
        ...
        <View style={styles.liftTracker}>
          ...
          <View style={styles.trackerCopy}>
            ...
            <Text style={styles.liftHint}>{liftedPhoneCount ? 'drop them onto the stack' : 'all phones are resting'}</Text>
          </View>
        </View>
