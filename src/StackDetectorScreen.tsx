...
export function StackDetectorScreen() {
  ...
  const { roomCode, isHost, playersArray, isAllReady, isStackVerified, createRoom, joinRoom, updateReadyState, sendShockwaveTimestamp } = useStackLobby(LOBBY_SERVER_URL);
  ...
  const localStatus = isLifted ? 'PHONE LIFTED' : isStackVerified ? 'STACK VERIFIED' : isAllReady ? 'WAITING FOR IMPACT' : isFaceDown ? 'READY ON STACK' : 'PLACE PHONE FACE DOWN';
