...
export function PhoneUsageBillScreen({ displayName, lobbyName: initialLobbyName, userId, roomCode: initialRoomCode, onSessionEnded }: Props) {
  ...
  const { isUsingPhone, billPercent, activeSeconds, recordScreenInteraction } = usePhoneUsageBill({ simulatedReading });
  ...
  const restingPhoneCount = roomCode
    ? playersArray.filter((player) => player.userId === activeUserId ? !isUsingPhone : !player.isUsingPhone).length
    : isUsingPhone ? 0 : 1;

  useEffect(() => {
    if (!roomCode) return;
    const reportStatus = () => reportPhoneUse(isUsingPhone).catch((error: Error) => setLobbyError(error.message));
    void reportStatus();
    const timer = setInterval(reportStatus, 1000);
  ...
  return (
    <SafeAreaView onTouchStart={recordScreenInteraction} style={styles.safeArea}>
      ...
      <ScrollView contentContainerStyle={[styles.screen, isWeb && styles.phoneFrame]} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.restingCard}>
          <View style={styles.phoneStack}><View style={styles.backPhone} /><View style={styles.frontPhone}><View style={styles.restEyes}><View style={styles.restEye} /><View style={styles.restEye} /></View></View><Text style={styles.zed}>z</Text><Text style={styles.bigZed}>z</Text></View>
          <Text style={styles.restingTitle}>shhh… {restingPhoneCount} phone{restingPhoneCount === 1 ? '' : 's'} resting</Text>
          <Text style={styles.restingHint}>phones that stay still keep the bill fair</Text>
        </View>
        ...
        <View style={styles.groupCard}>
          <Text style={styles.simulatorTitle}>LOBBY</Text>
          {roomCode ? <><View style={styles.lobbyHeader}><View style={styles.codeBoxes}>{roomCode.split('').map((digit, index) => <View key={`${digit}-${index}`} style={styles.codeBox}><Text style={styles.codeDigit}>{digit}</Text></View>)}</View><Text style={styles.groupHint}>{playersArray.length} person{playersArray.length === 1 ? '' : 's'} in lobby</Text></View><ScrollView horizontal contentContainerStyle={{ alignItems: 'flex-end', gap: 12, paddingTop: 10 }} showsHorizontalScrollIndicator={false}>{playersArray.map((player) => <View key={player.userId} style={{ alignItems: 'center', width: 58 }}><Text style={{ color: '#15121F', fontSize: 14, fontVariant: ['tabular-nums'], fontWeight: '800', marginBottom: 6 }}>{Math.round(player.billPercent)}%</Text><View style={{ backgroundColor: 'rgba(21,18,31,0.12)', borderColor: '#15121F', borderRadius: 8, borderWidth: 1, height: 128, justifyContent: 'flex-end', overflow: 'hidden', width: 28 }}><View style={{ backgroundColor: player.isUsingPhone ? '#F0908B' : '#86CBA3', height: `${Math.max(2, Math.min(100, player.billPercent))}%`, width: '100%' }} /></View><Text numberOfLines={1} style={{ color: '#15121F', fontSize: 11, fontWeight: '800', marginTop: 7, textAlign: 'center', width: 58 }}>{player.displayName}{player.userId === activeUserId ? ' (You)' : ''}</Text></View>)}</ScrollView></> : <><View style={styles.codeRow}><TextInput value={codeInput} onChangeText={(value) => setCodeInput(value.replace(/\D/g, '').slice(0, 4))} keyboardType="number-pad" maxLength={4} placeholder="0000" placeholderTextColor="#63708B" style={styles.codeInput} /><Pressable onPress={joinGroup} disabled={isJoining} style={styles.joinButton}>{isJoining ? <ActivityIndicator color="#FFFFFF" /> : <Text style={styles.joinText}>Join</Text>}</Pressable></View><Pressable onPress={createGroup} disabled={isJoining} style={styles.createButton}><Text style={styles.createText}>Create group</Text></Pressable></>}
